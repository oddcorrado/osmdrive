...
export function spawnSign(container, scene, x, y) {
   ...
   const rotSign = new Vector3(0, Math.PI/7*6, 0);
   const posSign = new Vector3(x, 0, y);
   const lineRot = new Vector3(Math.PI/2, Math.PI/7*6, y);
   const linePos = new Vector3(x, 1, y + 3.1);
   const showLinePos = new Vector3(x, -0.01, y + 3.1);
   const trigPos = new Vector3(x-5, 1, y+0.8);
